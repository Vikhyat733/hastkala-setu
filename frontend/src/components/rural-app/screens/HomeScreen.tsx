...
export const HomeScreen: React.FC<HomeScreenProps> = ({
  ...
  return (
    <div className="min-h-full flex flex-col justify-between bg-[#FAF6ED] text-[#2C241E] select-none">
      ...
      <div className="p-4 sm:p-5 space-y-5">
        ...
        <div className="flex items-center justify-between pt-1">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span className="absolute 1 top-1.5 right-1.5 w-2.5 h-2.5 bg-[#D95D39] rounded-full ring-2 ring-white animate-ping"></span>
              <span className="absolute 1 top-1.5 right-1.5 w-2.5 h-2.5 bg-[#D95D39] rounded-full ring-2 ring-white"></span>
            </button>
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-extrabold text-[#7D6E5D] bg-[#EADCC9]/50 px-2 py-0.5 rounded-lg">
              {artisan.name}
            </span>
          </div>
          <p className="text-base font-semibold text-[#5A4838] mt-0.5">
            आज क्या बेचना चाहेंगे?
          </p>
        </div>

        {/* GIANT HERO CARD: "नया सामान जोड़ें" (Add New Item) */}
        <div 
          onClick={() => {
            playSoundEffect('tap');
            onAddNewCraft();
          }}
          className="group relative cursor-pointer overflow-hidden rounded-3xl bg-gradient-to-b from-white to-[#F9F5EC] border-2 border-[#D7E8CC] p-5 sm:p-6 shadow-md hover:shadow-xl hover:border-[#3A6B35] transition-all transform active:scale-[0.98]"
        >
          {/* Subtle Background Accent */}
          <div className="absolute top-0 right-0 w-36 h-36 bg-[#E9F4E3]/60 rounded-full blur-2xl -mr-10 -mt-10 pointer-events-none"></div>
          ...
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-xs sm:text-sm font-bold text-[#5A6E50] mt-1 max-w-[260px]">
              फोटो लें और AI से सब कुछ बनवाएं
            </p>

            {/* Quick Touch Button */}
            <div className="mt-4 py-2.5 px-6 rounded-xl bg-[#3A6B35] text-white font-extrabold text-sm flex items-center gap-2 shadow-md shadow-[#3A6B35]/30">
        ...
        {/* SECTION: "मेरे सामान" (My Items Summary) */}
        <div className="pt-1">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-base font-extrabold text-[#3B3026]">
              मेरे सामान
            </h3>
            {products.length > 0 && (
              <button 
              ...
              >
                <span>सब देखें ({products.length})</span>
          ...
          {products.length === 0 ? (
            ...
            <div className="p-6 rounded-2xl bg-white border border-[#E5DAC8] text-center shadow-sm">
              ...
              <p className="text-sm font-bold text-[#6D5D4E]">
                कोई सामान नहीं है अभी
              </p>
              <p className="text-xs text-[#9E8E7F] mt-0.5">
                ऊपर दिए गए बटन से पहला सामान जोड़ें
              </p>
            </div>
          ) : (
            ...
            <div className="space-y-2.5">
              {products.slice(0, 2).map((item) => (
                <div
                  ...
                  className="p-3 rounded-2xl bg-white border border-[#E5DAC8] shadow-sm flex items-center justify-between cursor-pointer hover:border-[#3A6B35]/50 transition-all"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="inline-block mt-1 text-[10px] font-bold px-2 py-0.5 rounded-md bg-[#E8F0E3] text-[#3A6B35]">
                        {item.status === 'published' ? 'प्रकाशित (Live)' : item.status === 'order_received' ? 'ऑर्डर मिला!' : 'बिक गया'}
                      </span>
                    </div>
                  </div>
      ...
      <div className="p-3.5 mx-4 mb-3 rounded-2xl bg-[#E8F0E3] border border-[#3A6B35]/30 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <span className="text-lg">💡</span>
          <p className="text-xs font-bold text-[#2A4E26] leading-tight">
            AI आपके फोटो से शीर्षक, कीमत और विवरण खुद बना देगा
          </p>
        </div>
      </div>
