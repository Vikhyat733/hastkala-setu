...
import { ArrowLeft, Sparkles, Volume2, Heart } from 'lucide-react';
...
export const PublishConfirmScreen: React.FC<PublishConfirmScreenProps> = ({
  ...
  const handleVoiceAdvice = () => {
    ...
    if (activeLanguage === 'en') {
      speakText('Congratulations! Tap the green button below to publish your craft. Buyers across India will be able to discover and purchase it.');
    } else {
  ...
  return (
    <div className="min-h-full flex flex-col justify-between bg-[#FAF6ED] text-[#2C241E] select-none p-4 sm:p-6 max-w-xl mx-auto w-full">
      ...
      <div className="my-auto py-4 flex flex-col items-center text-center">
        ...
        <div className="relative mb-5">
          ...
          <div className="w-44 h-44 sm:w-52 sm:h-52 rounded-full bg-gradient-to-b from-[#E6F0DC] via-[#D8ECCE] to-[#FAF0DD] border-4 border-[#3A6B35]/30 p-2 flex flex-col items-center justify-center shadow-lg relative overflow-hidden">
            ...
            {/* Traditional Artisan Character doing Namaste */}
          ...
          <div className="mt-1 flex items-center justify-center gap-1.5 text-xs font-black text-[#2C5528]">
            ...
            <span>Radha Devi ({t('artisanLabel')})</span>
          </div>
        </div>
        ...
        <p className="text-xs sm:text-sm font-bold text-[#5A4838] mt-1.5 max-w-[300px]">
          {t('buyersAcrossIndia')}
        </p>

        {/* Item Mini Pill */}
        <div className="mt-4 px-4 py-2.5 rounded-2xl bg-white border border-[#E5DAC8] shadow-sm flex items-center gap-3">
          <img
            src={image}
            alt={name}
            className="w-10 h-10 rounded-xl object-cover border border-[#E5DAC8]"
          />
          <div className="text-left">
            <p className="text-xs font-black text-[#1E3A1E] leading-tight">{name}</p>
            <p className="text-xs font-extrabold text-[#3A6B35]">₹{price.toLocaleString('en-IN')}</p>
          </div>
        </div>
      ...
      <div className="pt-4">
        <button
          ...
          className="w-full py-4 px-6 rounded-2xl bg-[#3A6B35] hover:bg-[#2F582B] active:scale-[0.98] text-white font-extrabold text-xl flex items-center justify-center gap-2.5 shadow-xl shadow-[#3A6B35]/40 transition-all animate-pulse-slow"
        >
